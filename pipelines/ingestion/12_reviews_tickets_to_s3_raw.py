"""
pipelines/ingestion/12_reviews_tickets_to_s3_raw.py — Staff DE Journey

Production-grade ingestion from Reviews/Tickets local files → S3 Raw.

Real-world pattern:
  Reviews pulled daily from review platform (Trustpilot/Google API).
  Tickets pulled daily from helpdesk system (Zendesk/Freshdesk API).
  Two separate files per day — different systems, different schemas.
  Airflow pulls both at 02:00 AM daily.

File structure (local):
  /tmp/reviews_raw/YYYY/MM/DD/reviews_YYYYMMDD.json
  /tmp/reviews_raw/YYYY/MM/DD/tickets_YYYYMMDD.json

S3 partition structure:
  source=12_reviews_tickets/year=YYYY/month=MM/day=DD/
      reviews_YYYYMMDD.json
      tickets_YYYYMMDD.json

Silver join note:
  Reviews: order_id + product_sku → joined tuple from order_items ✅
  Tickets: order_id + customer_id → joined tuple from orders ✅
  Both consistent — no independent picks.

Watermark: manifest-based dedup
  s3://bucket/source=12_reviews_tickets/_metadata/uploaded_files.json

Usage:
  python 12_reviews_tickets_to_s3_raw.py
  python 12_reviews_tickets_to_s3_raw.py --dry-run
  python 12_reviews_tickets_to_s3_raw.py --input-dir /tmp/reviews_raw
"""
...
SOURCE_PREFIX = "source=12_reviews_tickets"
...
def build_s3_key(local_path: Path, input_dir: Path) -> str:
    ...
    Local:  /tmp/reviews_raw/2026/04/18/reviews_20260418.json
    S3:     source=12_reviews_tickets/year=2026/month=04/day=18/
                reviews_20260418.json

    No hour partition — daily batch source.
    """
    rel   = local_path.relative_to(input_dir)
    parts = rel.parts  # ('2026', '04', '18', 'reviews_20260418.json')
...
def run(input_dir: Path, dry_run: bool = False):
    ...
    log.info(f"Source 12 → S3 Raw | run_ts={run_ts.isoformat()}")
