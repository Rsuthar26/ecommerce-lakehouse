...
STREAM_SLEEP = 10.0 / 5  # 5 sensors, each fires every 10s = 1 event every 2s
TOPIC        = "iot.telemetry"
...
def sensor_reading(sensor: dict, dirty: bool) -> float | None:
    if sensor["type"] == "temperature":
        val = round(random.gauss(18.0, 2.0), 2)  # Normal warehouse temp
    elif sensor["type"] == "humidity":
        val = round(random.gauss(55.0, 5.0), 2)
    else:  # weight
        val = round(random.uniform(0, 5000), 2)   # kg on shelf

    if dirty and random.random() < 0.05:
        return random.choice([None, -999.0, 9999.0, 0.0])
    return val

def build_event(sensor: dict, event_dt: datetime, dirty: bool = False) -> dict:
    reading = sensor_reading(sensor, dirty)
    ...
    event = {
        ...
        "unit":         {"temperature": "celsius", "humidity": "percent", "weight": "kg"}.get(sensor["type"]),
    ...
    if dirty and random.random() < 0.02:
        event["ts"] = "2099-01-01T00:00:00+00:00"
...
def run_burst(days=7, dirty=False):
    ...
    # 5 sensors × 8640 readings/day × 7 days = 302,400 — sample 10K for burst
    total = days * 2000
    ...
    for i in range(total):
        ...
        event    = build_event(sensor, now - timedelta(days=days_ago), dirty)
        try:
...
def run_stream(dirty=False):
    ...
    try:
        while True:
            ...
            time.sleep(10)  # All sensors fire every 10 seconds
